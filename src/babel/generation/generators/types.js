...
export function Literal(node, print, parent) {
  var val  = node.value;
  var type = typeof val;
  // May not exist. Presumably in the case of nodes generated by babel.
  var raw  = node.raw;

  if (type === "string") {
    var token;

    // Possible Directive Prologue node, like Use Strict. Literal source may be
    // relevant, not just evaluated value.
    if (
      raw &&
      t.isExpressionStatement(parent)
    ) {
      token = acorn.tokenizer(raw).getToken();
    }
    ...
    if (
      ...
      // Skip quote normalization for this -- not worth the hassle.
      this.push(raw);
    }
    else this._stringLiteral(val);
  } else if (type === "number") {
